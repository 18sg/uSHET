/**
 * Implementation of EZSHET macros.
 *
 ...
 * name supplied as the callback/first variable name.
 *
 * Simple wrapper functions for registration and removal are defined which
 * EZSHET_ADD and EZSHET_REMOVE expand to.
 *
 * Wrapper functions for callbacks will be generated which
 * automatically type-check the incoming JSON and return sane error messages
 * (if applicable) if this fails. The JSON values are then unpacked and passed
 * to the user-defined callback function as conventional C arguments. The return
 * value of the user's callback may then also be packed and returned via SHET as appropriate.
 *
 * State variables are also defined such as appropriate shet_deferred_t structs
